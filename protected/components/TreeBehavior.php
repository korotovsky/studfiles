...
class TreeBehavior extends CNestedSetBehavior
{
    ...
    /**
     * Переименовывание. 
     * Зашкаливающее количество костылей в методе!!!
     */
    public function renameNode($node, $newname, $brother = null) {
        // Fetch nodes information
        $parent = $this;
        $transaction= $this->Owner->dbConnection->beginTransaction();
        try {
            $minleft = $parent->getRightValue();
            $minright = $parent->getRightValue();
            # Привет, программист! Извини меня за эту хуйню, честно. И в аргументах тоже.
            $sql = "UPDATE " . $this->Owner->tableName() . " SET `name` = '$newname' WHERE `id` = '" . (int) $_POST['node'] . "'";
            ...
            Yii::log("Error renaming node, transaction aborted. Exception: ".$e->getMessage(),"error","application.extensions.nestedset.treebehavior");
            $transaction->rollBack();
            return false;
        }
    }
